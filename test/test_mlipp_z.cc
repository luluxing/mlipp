...
TEST(MLIPPZTest, InitialConstructionAndSingleInsertion) {
  {
    MLIPP_Z<int> mlipp;
...
TEST(MLIPPZTest, BulkloadingAndExistingData) {
  {
    MLIPP_Z<int> mlipp;
  ...
  }
  {
    ...
    for (int i = 0; i < num; ++i) {
      ...
      data.push_back((Point<double>){x, y});
    }
    mlipp.bulk_load(data.data(), data.size());
    // Test existence
    for (int i = 0; i < num; ++i) {
...
TEST(MLIPPZTest, NonExistingData) {
  int num = 10000;
  {
    MLIPP_Z<int> mlipp;
    std::vector<Point<int>> data;
