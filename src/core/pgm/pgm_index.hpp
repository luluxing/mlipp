...
public:
    ...
    /**
     * Returns the key boundaries for leaf segments.
     * Each segment covers keys in the range [keys[i], keys[i+1]) for i < size-1,
     * and [keys[size-1], sentinel) for the last segment.
     * @return a vector of keys representing the start of each leaf segment
     * Author: LX
     */
    std::vector<K> get_leaf_segment_keys() const {
        std::vector<K> keys;
        if (segments.empty() || levels_offsets.size() < 2) {
            return keys;
        }
        
        size_t leaf_start = levels_offsets[0];
        size_t leaf_end = levels_offsets[1] - 1;  // Exclude sentinel
        
        keys.reserve(leaf_end - leaf_start);
        for (size_t i = leaf_start; i < leaf_end; ++i) {
            keys.push_back(segments[i].key);
        }
    ...
    /**
     * Returns the key of a specific leaf segment.
     * @param segment_index the index of the leaf segment (0 to segments_count()-1)
     * @return the key that starts this segment
     * Author: LX
     */
    K get_leaf_segment_key(size_t segment_index) const {
        if (segments.empty() || levels_offsets.size() < 2) {
            throw std::out_of_range("No segments available");
        }
        
        size_t leaf_start = levels_offsets[0];
        size_t leaf_end = levels_offsets[1] - 1;  // Exclude sentinel
        
        if (segment_index >= (leaf_end - leaf_start)) {
            throw std::out_of_range("Segment index out of range");
        }
        
        return segments[leaf_start + segment_index].key;
    }
};
